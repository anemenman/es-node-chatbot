module.exports = {
  ...
  index: async function (chatbot) {
    ...
    await client.create({
      ...
      body: {
        ...
        question_keywords: await module.exports.analyze(chatbot.question,
            client),
        type: chatbot.type,
  ...
  indexBulk: async function (chatbots) {
    ...
    var chatbotsBulk = [];
    for (var chatbot of chatbots) {
      var analyzeTokens = await module.exports.analyze(chatbot.question,
          client);
      chatbot.question_keywords = analyzeTokens.join(" ");
      chatbot.id = encode().value(chatbot.id);
    ...
    const body = chatbotsBulk.flatMap(
        doc => [{index: {_index: 'chatbot', _type: 'chatbot'}}, doc]);
  ...
  nextQuestion: function (history) {
    ...
    return result === undefined ? constants.NO_FOUND_ANSWER
        : constants.MAYBE_ANSWER + result.question;
  },
